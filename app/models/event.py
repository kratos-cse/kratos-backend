...
class EventRegistrationRule(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    event_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("events.id"), unique=True, nullable=False)
    ...
    fee_charge_model: Mapped[FeeChargeModel] = mapped_column(
        Enum(FeeChargeModel, name="fee_charge_model"), default=FeeChargeModel.PER_TEAM, nullable=False
    )
    ...
    member_registration_mode: Mapped[MemberRegistrationMode] = mapped_column(
        ...
        default=MemberRegistrationMode.LEADER_MANAGED,
        nullable=False,
    )
